...
{
    public partial class ActivosELN : System.Web.UI.Page
    {
        ...
        public string loadLogo(string symbol)
        {
            using (var ctx = new zestapp_dbEntities())
            {
                var logo = (from t in ctx.assets_logos where t.asset_code == symbol select t).FirstOrDefault();
                if(logo != null)
                {
                    return logo.img_url;
                }
